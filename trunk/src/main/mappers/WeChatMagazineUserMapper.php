...
class WeChatMagazineUserMapper
{
    ...
    public function getInfoByOpenId($openId, $includeUnSubscribe = false)
    {
        if ($includeUnSubscribe) {
            return $this->_db->getRow(
                "SELECT id, openid, unionid, nick_name, head_img, is_subscribe, phone, score, redpacket, create_time, update_time, invite, addr FROM wechat_magazine_user
                WHERE openid = ? ",
                $openId
            );
        } else {
    ...
    public function updateUserAddr($addr, $openId)
    {
        return $this->_db->execute(
            "UPDATE wechat_magazine_user SET addr = ? WHERE openid = ? ",
            array($addr, $openId)
        );
    }
}